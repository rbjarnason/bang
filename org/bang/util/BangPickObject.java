...
/**
 * Contains methods to aid in picking.  A BangPickObject is created
 ...
 */
public class BangPickObject extends Object {
   ...
   * @see Canvas3D
   */ 
  public BangPickObject(Canvas3D c, BranchGroup root)
    {
      pickRoot = root;
      canvas = c;      
    }
   ...
   * <code>BranchGroup</code> (specified in the BangPickObject constructor) that
   ...
   * <code>BranchGroup</code> (specified in the BangPickObject constructor) which
   ...
   * Pickable below the <code>BranchGroup</code> (specified in the BangPickObject
  ...
  public Node pickNode(SceneGraphPath sgPath, int flags)
	...
	if ((pickedNode instanceof Shape3D) && ((flags & SHAPE3D) != 0)){
	  ...
	  for (int j = 0; j < sgPath.nodeCount(); j++){
	    ...
	    else if ((pickedNode instanceof SharedGroup) && ((flags & LINK) != 0)){
	    ...
	    else if ((pickedNode instanceof UrlLink) &&
